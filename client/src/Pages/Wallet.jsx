...
import {
  ...
  useFetchWalletStellarTransactionHistroy,
} from '../Helpers/fetch';
...
function Wallet({ setSelectedCourse }) {
  ...
  const { data: dataHistroy, isFetching: isFetchingHistroy } =
    useFetchWalletStellarTransactionHistroy();
  useEffect(() => {
    if (dataHistroy) {
  ...
  const handleFundWallet = async () => {
    if (balance > 10000)
      return notify(
        'info',
        'Account balance already exceeds 10000 minimium balance'
      );
    if (loading) return;
    try {
      setLoading(true);
  ...
  const handleFundWithdrawal = async () => {
    if (balance < 20)
      return notify(
        'info',
        'Insufficient balance to make withdrawal, minimum balance is 20 XLM'
      );
    if (loading) return;
    try {
      setLoading(true);
    ...
    <PageLayout>
      <div className="bg-cream padx min-h-screen flex flex-col">
        ...
        <div className="flex items-center justify-center gap-[45px] flex-col">
          <div className="flex items-center flex-col justify-center gap-5">
            <div className="">
              {isFetching ? (
                ...
                <p className="text-[36px] text-gray-700 font-bold max-tablet:text-[21px]">
                  {balance} XLM
                </p>
              )}
            </div>
          ...
          {/**BTN TO FUND */}
          <div className="flex flex-col md:flex-row items-center justify-center gap-6 mb-12 mt-6">
            <div className="flex items-center justify-center gap-[22px]">
              <div onClick={handleFundWallet} className="btn">
                {loading ? 'Please Wait...' : 'Fund Wallet'}
              </div>
            </div>
            <div className="flex items-center justify-center gap-[22px]">
              <div
        ...
        {/**STELLAr Histroy */}
        <div className="mt-6">
          <h2 className="text-[21px] mt-12 font-semibold">
            Transaction Histroy
          </h2>
                ...
                <table className="min-w-full border-collapse text-sm">
                  ...
                  <tbody className="divide-y">
                    {transactionnHistroy?.map((tx) => (
                      <tr
                        key={tx.id}
                        className="px-6 bg-white text-[14px] font-medium text-cod-gray-80 hover:bg-gray-50 border-b-cod-gray-16"
                      >
                        <td className="p-3 ">
                          {tx?.id}
                          {/**
                        ...
                        <td className="p-3">
                          {tx?.fee_charged
                            ? (parseFloat(tx.fee_charged) / 1e7).toFixed(5)
                            : null}{' '}
                          xlm
                        </td>
                        <td className="p-3">{tx?.asset_type}</td>
                        <td className="p-3">
                          {truncateKey(tx?.to || tx?.account)}
                        </td>
                        <td className="p-3">
                          <span
